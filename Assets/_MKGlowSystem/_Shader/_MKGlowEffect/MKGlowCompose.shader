Shader "Hidden/MKGlowCompose" 
{
	...
	Subshader 
	{
		...
		Pass 
		{
			...
			uniform sampler2D _GlowTex : register (s1);
			...
			fixed4 frag( Output i ) : Color
			{
				...
				fixed d = i.uv[1].y;	
				d = (min(0,_GlowTex_TexelSize.y)/_GlowTex_TexelSize.y)*(1 - i.uv[1].y) + (max(0,_GlowTex_TexelSize.y)/_GlowTex_TexelSize.y)*(i.uv[1].y); //thanks to Ben Lindner <3 | OMG A EASTEREGG!!!
				i.uv[1].y = d;
